...
public:
    ...
    // ctor/dtor
    Sphere(float radius = 0.3f, int sectorCount = 36, int stackCount = 18, glm::vec3 amb = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 diff = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 spec = glm::vec3(0.5f, 0.5f, 0.5f), float shiny = 32.0f, glm::vec3 em = glm::vec3(0.05f, 0.0f, 0.0f)) : verticesStride(24)
    {
        //this->diffuseMap = dMap;
        //this->specularMap = sMap;
        set(radius, sectorCount, stackCount, amb, diff, spec, shiny, em);
    ...
    Sphere(unsigned int dMap, unsigned int sMap, float radius = 0.3f, int sectorCount = 36, int stackCount = 18, glm::vec3 amb = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 diff = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 spec = glm::vec3(0.5f, 0.5f, 0.5f), float shiny = 32.0f, glm::vec3 em = glm::vec3(0.05f, 0.0f, 0.0f)) : verticesStride(24)
    {
        this->diffuseMap = dMap;
        this->specularMap = sMap;
        set(radius, sectorCount, stackCount, amb, diff, spec, shiny, em);
    ...
    void set(float radius, int sectors, int stacks, glm::vec3 amb, glm::vec3 diff, glm::vec3 spec, float shiny, glm::vec3 em)
    {
        if (radius > 0)
            this->radius = radius;
    ...
    void setRadius(float radius)
    {
        if (radius != this->radius)
            set(radius, sectorCount, stackCount, ambient, diffuse, specular, shininess, emissive);
    }

    void setSectorCount(int sectors)
    {
        if (sectors != this->sectorCount)
            set(radius, sectors, stackCount, ambient, diffuse, specular, shininess, emissive);
    }

    void setStackCount(int stacks)
    {
        if (stacks != this->stackCount)
            set(radius, sectorCount, stacks, ambient, diffuse, specular, shininess, emissive);
    }
    ...
    void drawSphereWithTexture(Shader& lightingShader, glm::mat4 model) const      // draw surface
    {
        lightingShader.use();
